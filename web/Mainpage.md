...
-	Though arithmetic operations are internally rounded to single-precision using the underlying single-precision implementation's current rounding mode, those values are then converted to half-precision using truncation (round towards zero). This mixture of rounding modes is also the reason why std::numeric_limits<half_float::half>::round_style only returns `std::round_toward_zero` if the `float` specialization also does (which is very unlikely) and `std::round_indeterminate` otherwise.
